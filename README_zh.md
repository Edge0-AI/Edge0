...
edge0 把「在内存受限硬件上服务大 MoE」归约为三个可组合的机制：

- **SSD 专家 offload**。专家权重驻留磁盘、按需 mmap；LRU 维持激活集
  常驻，长尾专家按层预取。峰值内存由激活集而非参数量决定。
- **prerouter**。标准 MoE 解码中，第 *N* 层的专家选择依赖第 *N−1*
  层输出，每次专家装载都要等到路由决策之后、阻塞当前 step。prerouter
  是一个轻量训练头：用 token *t−1* 的隐层状态提前一层、提前一个 token
  预测第 *N* 层路由，使装载在 step 边界发出、I/O 延迟与前向计算重叠。
  对照同模型原生路由（同适配器、同负载），解码吞吐**最高 +59%**；
  收益随存储延迟与超出常驻容量的工作集占比增大——即随模型规模、
  路由宽度 *K* 与内存压力增大。
- **Recover-LoRA**。量化损失事后修复：冻结 int4 基模，挂载 LoRA 适配器，
  以 FP teacher 为目标、在真实与合成语料上（on-policy）用蒸馏损失训练。
  推理时适配器不合并——在只读、可多份 mmap 共享的基模上旁路叠加，
  一份基模服务多套适配器。在发布的 checkpoint 上，这使 4bit 质量与
  fp16 相差仅几分（见[质量](#质量)）。
