...
def _rope_interleave_torch(
    x: mx.array, positions: mx.array, theta: float
) -> mx.array:
    """Upstream ``apply_rotary_pos_emb_interleave``, verbatim from
    ``modeling_bailing_moe_v3`` (transformers 4.x ``default`` RoPE, which is
    what ``rope_scaling: null`` means for Ling-3.0-tiny):

        freqs = 1 / theta ** (arange(0, d, 2) / d)
        emb = cat((pos @ freqs, pos @ freqs))
        qi = q.view(d//2, 2).transpose(-1, -2).reshape(d)
        out = qi * cos + rotate_half(qi) * sin

    ``positions`` are absolute token ids (arange(L) + cache offset), matching
    torch's ``position_ids``/``cache_position``.  We cannot use
    ``mx.fast.rope`` for this: its ``offset`` is a single absolute position
    (vector must have 1 element), so a prefill with offset=0 silently skips
    rotation.  Verified against a float64 numpy replica of the upstream
    formula (rel ~1e-7, float32 rounding only).
    """
    B, H, L, D = x.shape
    freqs = 1.0 / (theta ** (mx.arange(0, D, 2, dtype=mx.float32) / D))
    angles = mx.outer(positions.astype(mx.float32), freqs)
    emb = mx.concatenate([angles, angles], axis=-1)
    cos = mx.cos(emb)[None, None, :, :]
    sin = mx.sin(emb)[None, None, :, :]
    xi = x.reshape(B, H, L, D // 2, 2).transpose(0, 1, 2, 4, 3).reshape(
        B, H, L, D
    )
    h = D // 2
    rh = mx.concatenate([-xi[..., h:], xi[..., :h]], axis=-1)
...
class ModelArgs(BaseModelArgs):
    ...
    # ---- pre-gated MoE (hybrid experiment) -----------------------------
...
class BailingMLA(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # RoPE follows the upstream torch implementation exactly:
        # transformers 4.x ``default`` (rope_scaling=null) computes
        # inv_freq = 1/theta^(2i/dim), emb = cat((freqs, freqs)), then
        # apply_rotary_pos_emb_interleave rotates consecutive pairs
        # (x[2i], x[2i+1]) by pos*inv_freq[i].  We hand-roll it
        # (_rope_interleave_torch) because mx.fast.rope's offset is a single
        # absolute position: prefill with offset=0 would skip rotation.
...
class BailingPrerouter(nn.Module):
    """One-MoE-block-ahead expert selector (arXiv 2308.12066 style).

    Input is the current layer's MoE input (post-attention RMSNorm output)
    plus optional current-layer top-8 one-hot and prev-token top-8 one-hot
    (v4/v5 feature set).  Output is 128 logits.  Selection reuses the
    sigmoid + group-limited top-k semantics of ``BailingGate`` (no expert
    bias), so at deployment the pre-gate can fully replace the router for
    its target layer.
    """
    ...
    def __init__(
        ...
        self.fc2 = nn.Linear(self.hidden, args.num_experts, bias=False)
        # v4/v5 residual linear path (zero by default; trained in v5).
        self.linear_init = nn.Linear(
            self.input_dim, args.num_experts, bias=False
        )
        self.linear_init.weight = mx.zeros(
            (args.num_experts, self.input_dim), dtype=mx.float32
        )

    def __call__(
        self,
        x: mx.array,
        topk_oh: mx.array | None = None,
        prev_oh: mx.array | None = None,
    ) -> mx.array:
        dtype = self.fc1.weight.dtype
        feats = [x.astype(dtype)]
        if topk_oh is not None:
            feats.append(topk_oh.astype(dtype))
        if prev_oh is not None:
            feats.append(prev_oh.astype(dtype))
        xi = mx.concatenate(feats, axis=-1)
        h = nn.gelu(self.fc1(xi))
        return self.fc2(h) + self.linear_init(xi)
...
class BailingSparseMoE(nn.Module):
    def __init__(
        self,
        args: ModelArgs,
        prerouter: BailingPrerouter | None = None,
        ...
        self.last_topk: mx.array | None = None
        self.prev_topk_oh: mx.array | None = None
        # v5 was trained with the TEACHER (original router) top-8 as the
        # pre-gate's "current layer top-8" feature, so at decode we compute
        # the original gate top-8 for the feature even when the pre-gate
        # selects the executed experts.  Set to "executed" to use the
        # pre-gate's own selection instead (post-SFT ce_only semantics).
        self.feature_topk = os.environ.get(
            "PREROUTER_FEATURE_TOPK", "teacher")
        # Same-token intra path computes the teacher feature inside the
        # after-attention callback; the per-layer gate re-computation here
        # would be dead work (nothing consumes last_topk in intra mode).
    ...
    def _select_from_logits(self, logits: mx.array) -> tuple[mx.array, mx.array]:
        """Same routing math as BailingGate, but from pre-gate logits
        (expert bias excluded)."""
        a = self.args
        ...
        if k_drop > 0:
            grouped = scores.reshape(
                *B, a.n_group, a.num_experts // a.n_group
            )
            top2 = mx.topk(grouped, 2, axis=-1)
            group_scores = top2.sum(axis=-1)
            drop = mx.argpartition(
                group_scores, kth=k_drop - 1, axis=-1
            )[..., :k_drop]
            masked = mx.put_along_axis(
                grouped,
                mx.expand_dims(drop, -1),
    ...
    def __call__(
        self, x: mx.array, prev_prerouter_logits: mx.array | None = None
    ) -> mx.array:
...
class BailingDecoderLayer(nn.Module):
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        self.prerouter_logits: mx.array | None = None
        self.m_in_cache: mx.array | None = None
        ...
        if layer_idx >= args.first_k_dense_replace:
            self.mlp = BailingSparseMoE(
                args,
                prerouter=(
                    BailingPrerouter(args)
                    if self.has_prerouter
                    else None
                ),
    ...
    def __call__(
        ...
        mask: mx.array | None = None,
        cache: Any | None = None,
        prev_prerouter_logits: mx.array | None = None,
    ) -> mx.array:
        h = x + self.attention(self.input_layernorm(x), mask, cache)
        m_in = self.post_attention_layernorm(h)
        if self.has_prerouter:
            # Defer the pre-gate MLP to a single post-eval batch (PrerouterStager
            # stacks every layer's m_in), keeping the forward graph lean.
            self.m_in_cache = m_in
...
class BailingModel(nn.Module):
    ...
    def __call__(
        ...
        prerouter_cache: dict[int, mx.array] | None = None,
    ) -> mx.array:
        h = (
            input_embeddings
        ...
        # Hidden-state clip (LING_HIDDEN_CLIP, e.g. 1000): clamps each
        ...
        for li, (layer, c) in enumerate(zip(self.layers, cache)):
            ...
            prev_prerouter_logits = None
            if (
                getattr(layer, "use_prerouter", False)
                and prerouter_cache is not None
                and li in prerouter_cache
            ):
                prev_prerouter_logits = prerouter_cache[li]
            h = layer(h, mask, c, prev_prerouter_logits)
            if _clip_v > 0:
                h = mx.where(
                    mx.isnan(h), mx.zeros_like(h),
                    mx.clip(h, -_clip_v, _clip_v))
